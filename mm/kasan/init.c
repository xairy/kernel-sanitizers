...
static inline bool kasan_pud_table(pgd_t pgd)
{
	return pgd_page(pgd) == virt_to_page(lm_alias(kasan_early_shadow_pud));
}
#else
static inline bool kasan_pud_table(pgd_t pgd)
{
...
static int __ref zero_pud_populate(pgd_t *pgd, unsigned long addr,
				unsigned long end)
{
					...
					const void *shadow_end)
{
	...
	do {
		...
		if (IS_ALIGNED(addr, PGDIR_SIZE) && end - addr >= PGDIR_SIZE) {
			 ...
			 * is noops.
			 */
			pgd_populate(&init_mm, pgd, lm_alias(kasan_early_shadow_pud));
			pud = pud_offset(pgd, addr);
			pud_populate(&init_mm, pud, lm_alias(kasan_early_shadow_pmd));
			pmd = pmd_offset(pud, addr);
...
static void kasan_free_pud(pud_t *pud_start, pgd_t *pgd)
{
	pud_t *pud;
	int i;
	...
	pud_free(&init_mm, (pud_t *)page_to_virt(pgd_page(*pgd)));
	pgd_clear(pgd);
}
...
void kasan_remove_zero_shadow(void *start, unsigned long size)
{
	...
	for (; addr < end; addr = next) {
		...
		kasan_free_pud(pud_offset(pgd, 0), pgd);
	}
}
