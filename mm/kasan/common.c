			...
			unsigned long *flags)
{
	...
	*size += sizeof(struct kasan_alloc_meta);
	...
	if (IS_ENABLED(CONFIG_KASAN_GENERIC) &&
	    (cache->flags & SLAB_DESTROY_BY_RCU || cache->ctor ||
 ...
 * SLAB_DESTROY_BY_RCU slabs to avoid use-after-free reports.
 ...
 * reallocation (even for non-SLAB_DESTROY_BY_RCU), because the constructor
...
{
	if (!cache->ctor && !(cache->flags & SLAB_DESTROY_BY_RCU))
